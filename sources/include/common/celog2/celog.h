...
#pragma once
#ifndef __CELOG2_CELOG_H__
#define __CELOG2_CELOG_H__
...
#define CELOG_MAX_LINE_LEN      10240
...
#define CELOG_LOGA(level, fmt, ...)                                     \
    CELog_LogA(CELOG_CUR_FILE, __LINE__, CELOG_CUR_MODULE, __FUNCTION__, level, fmt, \
              ##__VA_ARGS__)

#define CELOG_LOGW(level, fmt, ...)                                     \
    CELog_LogW(CELOG_CUR_FILE, __LINE__, CELOG_CUR_MODULE, __FUNCTION__, level, fmt, \
              ##__VA_ARGS__)
...
namespace CELog
{
  ...
  public:
    LogHelper(_In_      celog_filepathint_t fileInt,
      ...
      fn_(fn)
    {
      CELog_LogW(fileInt, line, mod, fn, CELOG_TRACE, L"Enter\n");
    }
  ...
  protected:
    ...
    // Cannot use std::string here since this class cannot have a destructor.
  ...
  public:
    ...
    {
      CELog_LogW(fileInt_, line, mod_, fn_, GetLeaveMsgLevel(), L"Leave%s\n",
                 GetLeaveMsgElapsedTimeStr().c_str());
    }

    template <class T>
    _Check_return_
    T RetT(_In_ int line, _In_ T val)
    {
      CELog_LogW(fileInt_, line, mod_, fn_, GetLeaveMsgLevel(),
#ifdef UNICODE
                 L"Leave, returning %ls%s\n",
#else
                 L"Leave, returning %hs%s\n",
#endif
                 val.ToString().c_str(),
                 GetLeaveMsgElapsedTimeStr().c_str());
      return val;
    ...
    template <class T>
    _Check_return_
    T &RetRT(_In_ int line, _In_ T &val)
    {
      CELog_LogW(fileInt_, line, mod_, fn_, GetLeaveMsgLevel(),
#ifdef UNICODE
                 L"Leave, returning %ls%s\n",
    ...
    #define DEF_FI_RET2_SAL(sal, type, fmt, printValExp)                \
      ...
      {                                                                 \
        CELog_LogW(fileInt_, line, mod_, fn_, GetLeaveMsgLevel(),       \
                   L"Leave, returning " fmt L"%s\n",                    \
                   (unsigned int) (sizeof(type) * 2), printValExp,      \
                   GetLeaveMsgElapsedTimeStr().c_str());                \
        return val;                                                     \
      }
  ...
  public:
    ...
    {
      RetOccurred();
      return FuncInvoke::RetT(line, val);
    }
    ...
    {
      RetOccurred();
      return FuncInvoke::RetRT(line, val);
    }
...
// Returning a value which has a member function ToString()
#define CELOG_RETURN_VAL_T(val)                                         \
  return fiwe.RetT(__LINE__, (val))

// Returning a reference which has a member function ToString()
#define CELOG_RETURN_REF_T(val)                                         \
  return fiwe.RetRT(__LINE__, (val))
...
#define CELOG_ENUM_ENTER(type)                                          \
  ...
  __LINE__, CELOG_CUR_MODULE, __FUNCTION__)
...
// Returning a value which has a member function ToString()
#define CELOG_SEH_RETURN_VAL_T(val)                                     \
  return fi.RetT(__LINE__, (val))

// Returning a reference which has a member function ToString()
#define CELOG_SEH_RETURN_REF_T(val)                                     \
  return fi.RetRT(__LINE__, (val))
...
static bool CELog_Init(void) {return true;}
static void CELog_Destroy(void) {}
...
#define CELOG_ENTER
#define CELOG_RETURN                        return
#define CELOG_RETURN_VOIDEXP(exp)           return exp
#define CELOG_RETURN_VAL(val)               return val
#define CELOG_RETURN_VAL_NOPRINT(val)       return val
#define CELOG_RETURN_VAL_T(val)             return val
#define CELOG_RETURN_REF_T(val)             return val
#define CELOG_ENUM_ENTER(type)
#define CELOG_ENUM_RETURN_VAL(val)          return val
#define CELOG_PTR_ENTER(type)
#define CELOG_PTR_RETURN_VAL(val)           return val

#define CELOG_SEH_ENTER
#define CELOG_SEH_RETURN                    return
#define CELOG_SEH_RETURN_VOIDEXP(exp)       return exp
#define CELOG_SEH_RETURN_VAL(val)           return val
#define CELOG_SEH_RETURN_VAL_NOPRINT(val)   return val
#define CELOG_SEH_RETURN_VAL_T(val)         return val
#define CELOG_SEH_RETURN_REF_T(val)         return val
#define CELOG_SEH_ENUM_ENTER(type)
#define CELOG_SEH_ENUM_RETURN_VAL(val)      return val
#define CELOG_SEH_PTR_ENTER(type)
#define CELOG_SEH_PTR_RETURN_VAL(val)       return val
