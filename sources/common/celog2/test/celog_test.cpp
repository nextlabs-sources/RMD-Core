...
#define CELOG_CUR_MODULE "CELogTest"
#define CELOG_CUR_FILE CELOG_FILEPATH_SOURCES_COMMON_CELOG2_TEST_CELOG_TEST_CPP
...
{
public:
  my_class_t(const std::string &first, const std::string &second) :
  ...
  {
    return "{first_ = " + first_ + ", second_ = " + second_ + "}";
  }
...
int LogLevelTest(void)
{
  ...
  ret = CELOG_LOG(CELOG_CRITICAL, _T("This is CRITICAL.\n"));
  if (ret < 0)
  {
  ...
  ret = CELOG_LOG(CELOG_ERROR, _T("This is ERROR.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG(CELOG_WARNING, _T("This is WARNING.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG(CELOG_INFO, _T("This is INFO.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG(CELOG_TRACE, _T("This is TRACE.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG(CELOG_DEBUG, _T("This is DEBUG.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG(CELOG_DUMP, _T("This is DUMP.\n"));
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(ret);
  ...
  ret = CELOG_LOG((celog_level_t) (CELOG_DUMP + 1),
                  _T("This is invalid log level.  This line should never be printed\n"));
  if (ret >= 0)
  {
...
int VarArgsTest(void)
{
  ...
  ret = CELOG_LOG(CELOG_INFO, _T("This is line one.\n"));
  ...
  ret = CELOG_LOG(CELOG_INFO, _T("%s %s %s %d!\n"), _T("This"), _T("is"), _T("line"), 2);
  printf("CELOG_LOG returned %d\n", ret);
  if (ret < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
...
int logOneMsgA(size_t logMsgLen)
{
  ...
  CELOG_RETURN_VAL(CELOG_LOGA(CELOG_CRITICAL, "len=%zu, str=\"%s\"\n",
                              logMsgLen, logMsg));
}
...
int logOneMsgW(size_t logMsgLen)
{
  ...
  CELOG_RETURN_VAL(CELOG_LOGW(CELOG_CRITICAL, L"len=%zu, str=\"%s\"\n",
                              logMsgLen, logMsg));
}
...
int MixedSbcsDbcsFmtArgsTest(void)
{
  ...
  if (CELOG_LOGW(CELOG_INFO, L"This is DBCS format with %hs string args.\n",
                 "SBCS") < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(-1);
  ...
  if (CELOG_LOGW(CELOG_INFO, L"This is DBCS format with %s string args.\n",
                 L"DBCS") < 0)
  {
    fprintf(stderr, "CELOG_LOG failed!");
    CELOG_RETURN_VAL(-1);
...
unsigned int __stdcall printLogThread(_In_ void *pParam)
{
  ...
  for (int i = 0; i < 100; i++)
  {
    if (CELOG_LOG(CELOG_DEBUG, _T("This is thread %d in the test.\n"),
                  * (int *) pParam) < 0)
    {
...
my_class_t NormalTestRetValT(void)
{
  CELOG_ENTER;
  my_class_t obj("Alice", "Bob");
  CELOG_RETURN_VAL_T(obj);
}

my_class_t& NormalTestRetRefT(void)
{
  CELOG_ENTER;
  static my_class_t obj("Carol", "Dan");
  CELOG_RETURN_REF_T(obj);
}
...
my_class_t SEHTestRetValT(void)
{
  CELOG_SEH_ENTER;
  my_class_t obj("Alice", "Bob");
  CELOG_SEH_RETURN_VAL_T(obj);
}

my_class_t& SEHTestRetRefT(void)
{
  CELOG_SEH_ENTER;
  static my_class_t obj("Carol", "Dan");
  CELOG_SEH_RETURN_REF_T(obj);
}
