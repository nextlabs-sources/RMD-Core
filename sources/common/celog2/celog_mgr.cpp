...
static const wchar_t* const levelTable[] =
{
...
void CELogMgr::PrintTimeZone(void)
{
  ...
  {
    fwprintf(logFile_, L"Local time zone is %s.  Timestamps below are in UTC.\n",
      dwRet == TIME_ZONE_ID_DAYLIGHT ? tziTest.DaylightName : tziTest.StandardName);
  }
}
...
void CELogMgr::ReadNxDir(void)
{
  ...
  WCHAR logDirPathBuf[MAX_PATH] = {0};

  if (!NLConfig::ReadKey(L"SOFTWARE\\NextLabs\\Logs\\LogDir",
                         logDirPathBuf, _countof(logDirPathBuf)))
  {
    wcsncpy_s(logDirPathBuf,_countof(logDirPathBuf),L"C:\\Program Files\\NextLabs\\SkyDRM\\diags\\logs",_TRUNCATE);
  }
  logDirPath_ = logDirPathBuf;
  if (logDirPath_.back() != L'\\')
  {
    logDirPath_ += L'\\';
  }
}
...
{
  // Get name of the module containing CELog2 in this process.  This is one of
  // the following:
  // 1. name of the .exe in this process (if the .exe uses CELog2)
...
bool CELogMgr::OpenLogFile(void)
{
  ...
  logFilePath << logDirPath_ << exeName_  << '_';
  ...
  logFileBasePath_ = logFilePath.str();
  
  logFilePath.str(L"");
  logFilePath << logFileBasePath_ << "0";

  logFile_ = _wfsopen(logFilePath.str().c_str(), L"at", _SH_DENYWR);
  if (logFile_ == NULL) {
    char errStr[1024];
    if (strerror_s(errStr, _countof(errStr), errno) == 0) {
      wchar_t msg[1024];
      if (swprintf_s(msg, L"%hs: Cannot create \"%s\": %hs\n", __FUNCTION__,
                     logFilePath.str().c_str(), errStr) != -1) {
  ...
  setvbuf(logFile_, NULL, _IONBF, 0);
  PrintTimeZone();
  return true;
}
...
{
    wstring logDir = logDirPath_;
    boost::filesystem::wpath logPath(logDir);

    // The code below calls three boost::filesystem functions:
    // A. exists()
    // B. is_directory()
    // C. wrecursive_directory_iterator constructor
    //
    // Their behavior in various error cases are:
    //
    // logDir                                           A           B           C
    // -----------------------------------------------  -------     -------     -------
    // C:\HasReadPermDir1                               return      return      return
    // C:\HasReadPermDir1\NonExistDir2                  return      return      excpt 3
    // C:\NoReadPermDir1                                return      return      excpt 5
    // C:\NoReadPermDir1\NonExistDir2                   return      return      excpt 3
    // C:\NoReadPermDir1\NonExistDir2\NonExistDir3      return      return      excpt 3
    // C:\NoReadPermDir1\NoReadPermDir2                 excpt 5     excpt 5     excpt 5
    // C:\NoReadPermDir1\NoReadPermDir2\NonExistDir3    return      return      excpt 3
    // C:\NoReadPermDir1\NoReadPermDir2\NoReadPermDir3  excpt 5     excpt 5     excpt 5
    // (3 = ERROR_PATH_NOT_FOUND, 5 = ERROR_ACCESS_DENIED)
    //
    // We just use one try-catch to catch all those exceptions.
    try
    {
        if ( boost::filesystem::exists(logPath) && boost::filesystem::is_directory(logPath))
        {
            ...
            while(it != endit && !deleteOldLogFileThreadStopFlag_)
            {
                if (is_regular_file(*it))
                {
                    ...
                    wstring fileName(it->path().filename().wstring());
    ...
    {
        // If we can't enumerate files in the log dir, forget it.
    }
}
...
bool CELogMgr::Init(void)
{
  if (initRefCount_++ > 0)
  {
    // Caller is not the first one to initialize it.
    return true;
  }
...
void CELogMgr::Destroy(void)
{
  if (--initRefCount_ > 0)
  {
    // Caller is not the last one to destroy it.
    return;
  }
...
int CELogMgr::Log(_In_          celog_filepathint_t fileInt,
                  ...
                  _In_          va_list argPtr)
{
  return LogReal(fileInt, line, mod, func, level, fmt, NULL, argPtr);
}
...
int CELogMgr::Log(_In_          celog_filepathint_t fileInt,
                  ...
                  _In_          va_list argPtr)
{
  return LogReal(fileInt, line, mod, func, level, NULL, fmt, argPtr);
}
...
bool CELogMgr::CheckRotation(void)
{
  ...
  if(sizeOfLog0+4096>=config_.logFileSize)
  {
    ...
    {
      numBuf.str(L"");
      numBuf << logFileBasePath_ << lastFile;
    ...
    do
    {
      ...
      logFilePath.str(L"");
      logFilePath << logFileBasePath_ << lastFile;

      logFileNextPath.str(L"");
      logFileNextPath << logFileBasePath_ << (lastFile+1);
...
int CELogMgr::LogReal(_In_              celog_filepathint_t fileInt,
                      _In_              int line,
                      _In_opt_z_        const char *mod,
                      _In_opt_z_        const char *func,
                      _In_              celog_level_t level,
                      ...
                      _In_              va_list argPtr)
{
  ...
  if (sbFmt != NULL)
  {
    ...
    ret1 = sprintf_s(sbcsLogLine, _countof(sbcsLogLine),
                     "%ls P%d T%lu F%d L%d %hs %hs %ls: ",
                     timestampStr, _getpid(), nlthread_selfID(), fileInt,
                     line, mod, func, levelTable[level]);
    if (ret1 == -1)
    {
      nlthread_mutex_unlock(&logMutex_);
      return -1;
  ...
  else
  {
    ...
    ret1 = swprintf_s(dbcsLogLine, _countof(dbcsLogLine),
                      L"%ls P%d T%lu F%d L%d %hs %hs %ls: ",
                      timestampStr, _getpid(), nlthread_selfID(), fileInt,
                      line, mod, func, levelTable[level]);
    if (ret1 == -1)
    {
  ...
  // If string was not truncated, return number of chars printed.
